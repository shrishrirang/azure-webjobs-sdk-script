    ...
    public class ScriptHost : JobHost
    {
            ...
            get
            {
                if (_instanceId == null)
                {
                    _instanceId = _settingsManager.GetSetting(EnvironmentSettingNames.AzureWebsiteInstanceId)
                        ?? Guid.NewGuid().GetHashCode().ToString("X").PadLeft(32, '0');
        ...
        protected virtual void Initialize()
        {
            ...
            using (metricsLogger.LatencyEvent(MetricEventNames.HostStartupLatency))
            {
                ...
                // _blobLeaseManager = blobManagerCreation.GetAwaiter().GetResult(); // FIXME: Check that changing Create from async to sync has no issues
