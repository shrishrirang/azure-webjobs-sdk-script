...
namespace Microsoft.Azure.WebJobs.Script
{
    internal sealed class BlobLeaseManager : IDisposable
    {
        ...
        public static BlobLeaseManager Create(ILeaseProxy leaseProxy, string accountName, TimeSpan leaseTimeout, string hostId, string instanceId, TraceWriter traceWriter)
        {
            ...
            return new BlobLeaseManager(leaseProxy, accountName, leaseTimeout, hostId, instanceId, traceWriter);
        }
        ...
        private async Task AcquireOrRenewLeaseAsync()
        {
            ...
            catch (LeaseException exc)
            {
                ...
                {
                    // If we did not have the lease already, a Conflict indicates that another host had it. This is
                    ...
                    // If we are here, it means the lease was 'stolen'. Log details for debugging.
                ...
                {
                    ProcessLeaseError($"Server error {exc}.");
                }
        ...
        private void TryReleaseLeaseIfOwned()
        {
            try
            {
                if (HasLease)
                {
                    ...
                    _leaseProxy.ReleaseLeaseAsync(leaseDefinition, CancellationToken.None).GetAwaiter().GetResult();
